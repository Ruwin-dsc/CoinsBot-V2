...
exports.run = async (bot, message, args, color) => {
    ...
    if (!args[1]) return message.reply(`:x: Merci de préciser un montant à retirer`)
    ...
    message.channel.send(`:coin: Vous venez de rertirer à ${mention.user.username} un montant de \`${args[1]} coins\``)
    } else if (args[0] == "rep") {
    const mention = message.mentions.members.first() || message.guild.members.cache.get(args[1]);
    if(!mention) return message.reply(`:warning: Utilisateur Invalide`)
    if (!args[2]) return message.reply(`:x: Merci de préciser un montant à retirer`)
    ...
    message.channel.send(`:small_red_triangle: Vous venez de de retier à ${mention.user.username} un montant de \`${args[1]} rep\``)
    }
